    ...
    def poll_peers(self) -> List[float]:
        ...
        for peer in self.peers:
            try:
                ...
                logger.warning(f"Failed to poll NTP peer {peer}: {type(e).__name__}: {e}")

        if not offsets:
            message = "NTP queries failed for all peers"
...
# Default NTP peers (mix of server IPs and hostnames)
DEFAULT_NTP_PEERS = [
