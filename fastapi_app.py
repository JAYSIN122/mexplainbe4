...
def _closure_rate(t_list, deg_series, min_days=150, max_days=300):
    """Estimate recent closure rate from phase history.

    Parameters
    ----------
    t_list : list[datetime]
        Timestamps corresponding to the phase measurements.
    deg_series : sequence[float]
        Phase gap values in degrees.
    min_days : int, optional
        Minimum look-back window in days. Currently unused but kept for
        API symmetry with ``_robust_fit_eta``.
    max_days : int, optional
        Maximum look-back window in days used to select the recent window.

    Returns
    -------
    dict or None
        ``{"slope_rad_per_day": float, "phase_gap_rad": float}`` if a
        reliable fit could be produced, otherwise ``None``.
    """

    if len(t_list) < 20:
        return None

    # Convert degrees to unwrapped radians so the regression operates on a
    ...
    # Iteratively fit and remove outliers similar to ``_robust_fit_eta``.
    for _ in range(2):
    ...
    coeffs = np.polyfit(x, y, 1)
    slope = float(coeffs[0])  # radians per day
    phi_now = float(y[-1])

    return {"slope_rad_per_day": slope, "phase_gap_rad": phi_now}
